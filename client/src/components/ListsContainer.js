  ...
  render() {
    let lists = this.state.lists.map((list, index) => {
      if (this.state.editingListId === list.id) {
        return (<EditListForm list={list} key={index} editList={this.editList} />)
      } else {
        return (<List list={list} key={index} onRemoveList={this.removeList} editingList={this.editingList} />)
      }
    })
